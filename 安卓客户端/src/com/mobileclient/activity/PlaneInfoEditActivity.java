...
public class PlaneInfoEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.planeinfo_edit); 
			...
			public void onClick(View v) {
				try {
					...
					if(ET_planeNumber.getText().toString().equals("")) {
						Toast.makeText(PlaneInfoEditActivity.this, "汽车站输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_price.getText().toString().equals("")) {
						Toast.makeText(PlaneInfoEditActivity.this, "票价输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_seatNumber.getText().toString().equals("")) {
						Toast.makeText(PlaneInfoEditActivity.this, "总座位数输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_leftSeatNumber.getText().toString().equals("")) {
						Toast.makeText(PlaneInfoEditActivity.this, "剩余座位数输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_startTime.getText().toString().equals("")) {
						Toast.makeText(PlaneInfoEditActivity.this, "发车时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_endTime.getText().toString().equals("")) {
						Toast.makeText(PlaneInfoEditActivity.this, "终到时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_totalTime.getText().toString().equals("")) {
						Toast.makeText(PlaneInfoEditActivity.this, "历时输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传汽车站信息信息*/
					PlaneInfoEditActivity.this.setTitle("正在更新汽车站信息信息，稍等...");
					String result = planeInfoService.UpdatePlaneInfo(planeInfo);
					Toast.makeText(getApplicationContext(), result, 1).show(); 
					/*操作完成后返回到汽车站信息管理界面*/ 
					Intent intent = new Intent();
					intent.setClass(PlaneInfoEditActivity.this, PlaneInfoListActivity.class);
					startActivity(intent); 
					PlaneInfoEditActivity.this.finish();
				} catch (Exception e) {}
			}
		});
	}
	...
	private void initViewData() {
		...
		this.ET_planeNumber.setText(planeInfo.getPlaneNumber());
		for (int i = 0; i < stationInfoList.size(); i++) {
			if (planeInfo.getStartStation() == stationInfoList.get(i).getStationId()) {
				this.spinner_startStation.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < stationInfoList.size(); i++) {
			if (planeInfo.getEndStation() == stationInfoList.get(i).getStationId()) {
		...
		for (int i = 0; i < seatTypeList.size(); i++) {
			if (planeInfo.getSeatType() == seatTypeList.get(i).getSeatTypeId()) {
				this.spinner_seatType.setSelection(i);
				break;
			}
		}
		this.ET_price.setText(planeInfo.getPrice() + "");
		this.ET_seatNumber.setText(planeInfo.getSeatNumber() + "");
		this.ET_leftSeatNumber.setText(planeInfo.getLeftSeatNumber() + "");
		this.ET_startTime.setText(planeInfo.getStartTime());
		this.ET_endTime.setText(planeInfo.getEndTime());
		this.ET_totalTime.setText(planeInfo.getTotalTime());
	}
	@Override
